...
public:
    ...
    */
    ...
    void rotateAndMatch(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> initScan, boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> target, float &score, float *trans)
    {
    ...
    void relocation()
    {
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr initScanTrans(new pcl::PointCloud<pcl::PointXYZ>());
        ...
        //选取最近路径点前后10个点作匹配
        vector<float*> eachTrans,localTrans;
        vector<float> eachScore,localScore;
        
        pcl::PointCloud<pcl::PointXYZ>::Ptr target(new pcl::PointCloud<pcl::PointXYZ>());
        std::vector<float> rotateAngle;
        for(int i=0;i<relocationRotateGap;i++)
        {
            rotateAngle.push_back(i*2*M_PI/relocationRotateGap);
        }
        for(int i=searchIndex[0]-(int)(relocationKeyMapSize/2);i<searchIndex[0]+(int)(relocationKeyMapSize/2);i+=relocationPickGap)
        {
            ...
            *target+=pointxyzi_to_pointxyz(surfVector[i]);
            ...
            // for(int n=0;n<rotateAngle.size();n++)
            // {
            //     float* localTrans=new float[6];
            //     localTrans[0]=0.0;
            //     localTrans[1]=0.0;
            //     localTrans[2]=rotateAngle[n];
            //     localTrans[3]=nearestP.x;
            //     localTrans[4]=nearestP.y;
            //     localTrans[5]=nearestP.z;
            //     Eigen::Affine3f T=trans2Affine3f(localTrans);
            //     // cout<<"rev init rpy:"<<T.rotation().eulerAngles(0,1,2).transpose()<<endl;
            //     transformPointCloud(initScan,T,initScanTrans);
            //     pcl::PointCloud<pcl::PointXYZ>::Ptr matchResult(new pcl::PointCloud<pcl::PointXYZ>());
            //     if(useIcp==0)
            //     {
            //         icpProcess(initScanTrans,target,score,trans,matchResult);
            //     }
            //     else if(useIcp==1)
            //     {
            //         ndtProcess(initScanTrans,target,score,trans,matchResult);
            //     }
            //     else if(useIcp==2)
            //     {
            //         gicpProcess(initScanTrans,target,score,trans,matchResult);
            //     }
            ...
            //     trans[3]+=localTrans[3];
            //     trans[4]+=localTrans[4];
            //     trans[5]+=localTrans[5];
            ...
            if(cloudInfo.imuAvailable)
            {
                ...
                localTrans[0]=cloudInfo.imuRollInit;
                localTrans[1]=cloudInfo.imuPitchInit;
                localTrans[2]=cloudInfo.imuYawInit-M_PI_2;
            }
            localTrans[3]=nearestP.x;
            localTrans[4]=nearestP.y;
            localTrans[5]=nearestP.z;
